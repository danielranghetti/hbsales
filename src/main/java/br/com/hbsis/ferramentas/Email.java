...
public class Email {
    ...
    public void enviarEmailDataRetirada(Pedido pedido, List<Item> items) {
        ...
        message.setText(pedido.getFuncionario().getNome()  + "\r\n"
                + "O seu pedido: " + pedido.getCodPedido() + "\r\n" +
                "Do fornecedor: "+ pedido.getPeriodoVenda().getFornecedor().getRazaoSocial() + "foi aprovado" + "\r\n" +
                 "Com os seguintes itens: " + "\r\n" +
                listaItens(items)+
                "A data de retirado do seu pedido é " + pedido.getData().plusDays(10) + " ultima data para retira " +pedido.getPeriodoVenda().getDataRetirada().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "\r\n" +
